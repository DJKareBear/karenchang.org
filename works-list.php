...
<?php
    ...
    if($genre_template === true){
        ...
        if( ! empty( $term_slug ) ){
            $args[CPT_WORK_TAXONOMY] = $term_slug;
            $term_name = get_term_by( 'slug', $term_slug, CPT_WORK_TAXONOMY);
        }
    }
...
<?php
    ...
    elseif ($wp_query->have_posts()) :
        ...
        $zoom_class = $apollo13->get_option('cpt_work', 'hover_zoom' ) == 'on'? ' hov-zoom' : '';

        echo '<div id="a13-works"'.$no_resize_param.' class="'.$apollo13->get_option('cpt_work', 'hover_type' ).$zoom_class.'">';

        while ( have_posts() ) :
            the_post();

            //get album genres
            $terms = wp_get_post_terms(get_the_ID(), CPT_WORK_TAXONOMY, array("fields" => "all"));
            $pre = 'data-genre-';
            $suf = '="1" ';
            $genre_string = '';
            ...
            echo '<i>'.a13_make_work_image($post->ID, 'work-cover' ).'</i>';
            echo '<em class="cov"><span>' . a13_subtitle('small') . get_the_title() . '</span></em>';
            echo '</a>';
        endwhile;
